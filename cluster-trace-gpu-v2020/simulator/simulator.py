...
class Simulator:
    def __init__(self, csv_file, alloc_policy=0, preempt_policy=0,
                 ...
                 dataset=None, num_jobs_limit=None, gpu_type_matching=0, verbose=0):
        self.cluster = None
        self.scheduler = None
        self.cur_time = None
        self.num_jobs = None
        self.job_list = copy.deepcopy(dataset)
        self.job_runn_list = []
        self.exit_flag = None
        ...
        self.job_origin_list = dataset #self.add_job(self.csv_file, self.describe_dict, limit=num_jobs_limit * 10)
    ...
    def simulator_go(self, repeat=1, num_jobs=None):
        ...
        for repeat_id in range(repeat):
            ...
            with open('jobs_'+str(self.alloc_policy)+'_'+str(self.sort_node_policy)+'.csv', mode='w') as file:
                ...
                for job in jobbe:
                    writer.writerow([job['job_id'], job['submit_time'], job['duration'], job['jct'], job['num_inst'], job['gpu_type'], job['num_cpu'], job['num_gpu'], job['size'], job['wasted'], job['resource'], job['node'], job['allocated_at'], abs(job['submit_time']-job['allocated_at']), job['progress']])
