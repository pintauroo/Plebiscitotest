...
from Alibaba.utils import print_fn
...
class Node(object):
    ...
    def release_gpus(self, num_gpus=0):
        ...
        '''
        if round(float(self.idl_gpus + num_gpus), 2) <= round(float(self.num_gpus), 2):
            self.idl_gpus += num_gpus
            return True
        else:
    ...
    def release_cpus(self, num_cpus=0):
        ...
        '''
        if round(float(self.idl_cpus + num_cpus), 2) <= round(float(self.num_cpus), 2):
            self.idl_cpus += num_cpus
            return True
        else:
